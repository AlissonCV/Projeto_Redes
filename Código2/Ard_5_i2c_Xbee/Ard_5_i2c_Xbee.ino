...
int cmd = 0;  //Definição de variáveis carecteres
...
ZBRxResponse rx = ZBRxResponse(); // Atribui uma variável de recebimento de dados
...
void loop() {
  ...
  receiveXbee(); // Quando requisitado dados do arduino 3 (Xbee)
...
void receiveXbee() {
  ...
  if(xbee.getResponse().isAvailable()){
    if(xbee.getResponse().getApiId()==ZB_RX_RESPONSE){
      xbee.getResponse().getZBRxResponse(rx); // Atribui ao recebimento o pacote
      resposta = (int)rx.getData(0); // Atribui a variável resposta a mensagem
    }
  }
