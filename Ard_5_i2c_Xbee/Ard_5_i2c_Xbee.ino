...
void loop() {
  requestEvent(); //Requisição de dados do arduino 4 e 3 (Escravo/Xbee)
...
void requestEvent() {
  ...
  if (resposta == 255) {
    payload[0] = (176 | ((b1&3)<<2) | (b0&3)); // Carrega os dados para envio
    xbee.send(data); // Envio do dado do sensor B0 e B1 via Wirelees
  }
  else if (((resposta>>4)&15) == 11) {
    digitalWrite(rele, (resposta & 3)); //Escrita na porta do rele para controle do solenoide
  }
}