...
char num[2]= {0, 0};  //Definição de variáveis carecteres
uint8_t b0, b1;
...
void loop() {
  ...
  payload[0] = ((b0&3)<<2) | (b1&3)); // Carrega os dados para envio
  xbee.send(data); // Envio do dado do sensor B0 e B1 via Wirelees
  
  reciveXbee(); //Recebimento de dados de comando via Xbee do arduino 3

  if (num[0]) {
    digitalWrite(rele, num[1]); //Escrita na porta do rele para controle do solenoide
    num[0] = 0; //Bloqueio de acionamento repetitivo do rele
  }
...
void reciveXbee() {
  ...
  if(xbee.getResponse().isAvailable()){
    if(xbee.getResponse().getApiId()==ZB_RX_RESPONSE){
      ...
      num[0] = 1; // Atribuição do valor do sensor B0 na leitura dos dados enviados pelo Arduino 5
    }
  }

  num[1] = (resposta & 1); // Atribuição do valor do sensor B1 na leitura dos dados enviados pelo Arduino 5
}